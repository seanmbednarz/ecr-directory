...
import { Person, DEPTS, byId, kidMap, colorOf, initials } from './data';
...
export default function PersonDrawer({ person, onClose, onSelect }: Props) {
  const open = !!person;
  const dept = person ? DEPTS[person.dept] : null;
        ...
        {person && (
          <>
            ...
            <div className="px-6 pt-6 text-center flex-shrink-0">
              ...
              {dept && (
                <span
                  className="inline-flex items-center gap-1.5 mt-2.5 text-xs font-bold tracking-wide uppercase px-3 py-1 rounded-full"
                  style={{
                    color,
